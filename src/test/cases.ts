...
export function workspaceFile({ kind, server, active }: Launch): WorkspaceFile {
	...
	const servers: Record<string, IJSONServerSpec> = {
		[server.serverName]: { webServer: { scheme: "http", host: "localhost", port: server.port, pathPrefix: "" }, ...credentials },
	};
	switch (kind) {
		case "clientSide-os-host":
			return { ...client, settings: { "objectscript.conn": { ...conn, https: false, host: "localhost", port: server.port, ...credentials } } };
		...
		case "clientSide-sm":
			return { ...client, settings: { "objectscript.conn": { ...conn, server: server.serverName }, "intersystems.servers": servers } };
		case "serverSide-sm":
